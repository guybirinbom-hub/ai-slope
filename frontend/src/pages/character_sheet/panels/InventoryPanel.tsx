        ...
        <Group>
          ...
          {isPhone ? (
            ...
            <Group wrap='nowrap'>
              ...
              <MonsterPartsBadge entity={props.entity} setEntity={props.setEntity} />
              <ImprintButton
                radius='xl'
                size='sm'
                fw={500}
...
// Pseudo-currency badge for the Battlezoo Monster Parts subsystem. Only
// renders when the character has the `monster_parts` variant enabled.
// Click → popover with a NumberInput for the gp-value of monster parts
// the character currently holds, plus a free-form notes field for
// recording what monster they came from (the gathering rules ask the
// player to note the source so they know what imbued-property
// requirements the parts satisfy).
//
// Stored at `inventory.monster_parts.{value, notes}` — a separate
// optional field on the Inventory schema so non-Battlezoo games leave
// it absent.
function MonsterPartsBadge(props: {
  entity: LivingEntity | null;
  setEntity: any;
}) {
  // Variants live on Character, not LivingEntity in general. The badge
  // only renders for characters with the variant on.
  const hasVariant =
    (props.entity as unknown as { variants?: { monster_parts?: boolean } } | null)?.variants?.monster_parts === true;
  if (!hasVariant) return null;

  const mp = props.entity?.inventory?.monster_parts;
  const value = mp?.value ?? 0;
  const notes = mp?.notes ?? '';

  const update = (next: Partial<{ value: number; notes: string }>) => {
    props.setEntity((prev: any) => {
      if (!prev) return prev;
      return {
        ...prev,
        inventory: {
  ...
  return (
    <Popover width={280} position='bottom' withArrow shadow='md' withinPortal>
      <Popover.Target>
        <Badge
          variant='light'
          color='teal'
        ...
        >
          MP: {value.toLocaleString()} gp
        </Badge>
      </Popover.Target>
      <Popover.Dropdown>
        <Stack gap={6}>
          <Text fz='sm' fw={600} c='gray.2'>
            Monster Parts (Battlezoo)
          </Text>
          <NumberInput
            size='xs'
            label='Value (gp)'
            ...
            max={10_000_000}
            step={10}
            onChange={(v) => update({ value: typeof v === 'number' ? v : 0 })}
          />
          <Textarea
            size='xs'
            label='Sources / notes'
            placeholder='e.g. "120 gp from a young red dragon, fire trait"'
            value={notes}
          ...
          <Text fz='xs' c='gray.5' fs='italic'>
            Spend these via the Battlezoo Monster Parts panel on each item (Refinement value / Imbued properties).
          </Text>
        </Stack>
      </Popover.Dropdown>
    </Popover>
