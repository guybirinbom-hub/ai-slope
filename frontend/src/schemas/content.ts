...
export interface Item {
  ...
  meta_data: {
    ...
    // Battlezoo Monster Parts state for this item. Only present when the
    // owner has the `monster_parts` character variant enabled AND has
    // explicitly flipped this item into monster-parts mode. While
    // `enabled` is true, the item's fundamental rune fields
    // (potency/striking/resilient) are auto-derived from
    // `refinement_value` via the refinement tables (3A for weapons /
    // armor, 3B for shields / perception / skill items). The user
    // doesn't edit potency/striking directly in this mode. Property
    // runes still come from `runes.property` — but the picker UI
    // restricts to Battlezoo Imbued Property items.
    //
    // `category` lets the player override the auto-detected category
    // for items whose group is GENERAL (perception items, skill items).
    // `skill_variable` is the matching skill variable (e.g. SKILL_ARCANA)
    // when category === 'skill', used so the refinement bonus lands on
    // the right skill.
    battlezoo?: {
      enabled: boolean;
    ...
    meta_data: z
      .object({
        ...
        cleaning: z
          ...
          .optional(),
        // Battlezoo Monster Parts state — see the matching block on the
        // Item TS interface above for full semantics.
        battlezoo: z
          .object({
            enabled: z.boolean(),
...
export const InventorySchema = z.object({
  ...
  // Battlezoo Monster Parts (Light variant): an opt-in pseudo-currency
  // tracking the gp-value of unspent monster parts in the character's
  // pool. Optional so existing characters / non-Battlezoo games leave
  // it absent. Only the Monster Parts UI panel reads / writes this.
  monster_parts: z
    .object({
      value: z.number(),
...
const CharacterVariantsSchema = z.object({
  ...
  // Deep Background variant: instead of picking a published background,
  // the player builds a custom one. Mechanically it grants 2 attribute
  // boosts to different scores, training in a Lore skill, and one skill
  // feat (whose prereq skill the character is automatically trained in).
  // When this is on, the BackgroundAccordionItem in the builder is
  // replaced by a custom form that writes operations into
  // character.custom_operations + a synthesized background record.
  deep_background: z.boolean().optional(),
  // Battlezoo Monster Parts subsystem (Light variant). When on, each
  // item in the character's inventory can be flipped into "monster
  // parts" mode (see Item.meta_data.battlezoo). In that mode, the
  // item's refinement value (gp of monster parts invested) drives
  // potency/striking/resilient instead of fundamental runes, and
  // property-rune slots accept Battlezoo Imbued Property items only.
  // Toggle controls visibility / availability of the per-item
  // monster-parts panel.
  monster_parts: z.boolean().optional(),
});
  ...
  meta_data: z
    .object({
      ...
      calculated_stats: z
        ...
        .optional(),
      // Per-character "favorites" list — entries the player has
      // starred from any drawer popup. Shown in a Favorites accordion
      // above Weapon Attacks on the Actions panel for quick access.
      // `type` matches DrawerType ('feat' | 'action' | 'inv-item' |
      // 'class-feature' | 'spell' | 'item' | ...). For inv-items `id`
      // is the inventory item id (string); for everything else it's
      // the content-row id (number). `name` is stored so the bar can
      // render without re-fetching content.
      favorites: z
        .array(
          z.object({
            type: z.string(),
    ...
    .object({
      ...
      info: z
        .object({
          ...
          // Deep Background variant data (see CharacterVariantsSchema.deep_background).
          // Persisted here so the form remembers the player's choices when
          // the variant toggle gets flipped off and back on.
          deep_background: z
            .object({
              name: z.string().optional(),
              description: z.string().optional(),
              boost1: z.string().optional(), // 'STR' | 'DEX' | 'CON' | 'INT' | 'WIS' | 'CHA'
              boost2: z.string().optional(),
              lore_name: z.string().optional(), // e.g. "Cooking" → SKILL_LORE___COOKING
              skill_feat_id: z.number().optional(),
              prereq_skill: z.string().optional(), // chosen when feat prereq has options
            })
            .optional(),
        })
        .optional(),
