...
import { applyMonsterPartsToItem } from './monster-parts';
...
export function getWeaponStats(id: StoreID, item: Item) {
  // Battlezoo Monster Parts: if the item is in monster-parts mode,
  // its fundamental-rune fields (potency / striking / resilient) are
  // derived from refinement_value via Tables 3A/3B + 4A/4B. The rest
  // of this function reads `item.meta_data.runes.*` directly, so we
  // apply the derivation here so the attack bonus / damage dice
  // displays match what the character compute applies.
  if (item.meta_data?.battlezoo?.enabled) {
    item = applyMonsterPartsToItem(item, id);
  }
