...
export default function RichText(props: RichTextProps) {
  ...
  // Resolve `[label](pf:TYPE:NAME)` sentinels into real content links
  // via the cached content store. This lets hand-authored homebrew
  // content (e.g. the Battlezoo imbued-property item descriptions)
  // reference spells / feats / actions / etc. by NAME without the
  // generator having to know real content row ids. If the lookup
  // fails (cache cold or unknown name) the label is rendered as
  // plain text — same fallback behavior as convertToHardcodedLink.
  convertedChildren = convertedChildren?.replace(
    /\[([^\]]+)\]\(pf:([a-z-]+):([^)]+)\)/g,
    (_match, label, type, name) => convertToHardcodedLink(type, name, label)
  );
